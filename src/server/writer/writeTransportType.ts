...
const writeTransportType = (feed: XMLDocumentCB, adapterName: string): void => {
  const transportType = lookup[adapterName];

  if (!transportType) {
    throw new Error(
      `Error when writing TransportType. Unknown adapter name ${adapterName}`,
    );
  }
...
export default writeTransportType;
